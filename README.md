...
thx qwen .4.1 Mythos 20b model

performance hooks for tf 2. injects a dl into the game, hooks client-side functions that cost a lot on weak CPUs, and exposes a CLI so you can turn each one or off while playing.
...
`menuoff` is on by default: every setting suspends outside a match, so menus behave stock.
It needs both engine signals (`IsInGame` and `GetLocalPlayer`) to agree before it suspends
anything, and `status` shows `in_game` so you can see what it thinks. Type `help` for the
command list, `quit` to exit.
...
Type these in the CLI after `tf2perf inject`. `alloff` turns everything off.
...
Aggressive (more culling, no HUD flicker, weapons always render):
