...
`menuoff` is off by default. If a menu ever breaks for you, `menuoff on` suspends every
setting outside a match - but check `status` first, `in_game=0` during a match means the
engine check is misreading on your build and you should leave it off. Type `help` for the
