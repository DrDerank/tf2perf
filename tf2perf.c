...
static volatile LONG g_menuoff         = 0;   // 1 = suspend all settings outside a match
                                              // (off by default: engine detection can misread)
...
static void cmd_help(char* out, size_t cap)
{
    append(out, cap,
        ...
        "  menuoff <on|off>          suspend every setting outside a match. off by default;\n"
        "                            only works if engine detection is right (check status)\n"
